...
public class HighScoreManager : MonoBehaviour {
	...
	private const string privateCode = "PASTE PRIVATE CODE HERE";
	private const string publicCode = "5adc41fed6024519e0e56037";
	...
	private IEnumerator DownloadHighScoresFromDatabase() {
		...
		WWW www = new WWW(webURL + publicCode + "/pipe/50");
		yield return www;
		...
		for(int i = 0; i < 50 - scores.Length; i++) {
			nameList += "???\n";
			scoreList += "0\n";
		}
	...
	private Highscore[] FormatHighscores(string textStream) {
    	...
    	lowestScore = entries.Length < 50 ? 0 : highscoresList[entries.Length - 1].score;
